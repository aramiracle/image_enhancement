...
class ImageEnhancementDataset(Dataset):
    ...
    def add_gaussian_blur(self, image, kernel_size=5, sigma=1):
        if random.random() < 0.7:  # Apply blur with 50% probability
            image = transforms.GaussianBlur(kernel_size, sigma)(image)
        return image

    def add_gaussian_noise(self, image, mean=0, std=0.005):
        if random.random() < 0.7:  # Apply noise with 50% probability
